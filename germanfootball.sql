CREATE OR REPLACE VIEW dfbpokal_fixtures AS
SELECT * FROM "2024_25_german_cup__first_round_fixtures"
UNION ALL
SELECT * FROM "2024_25_german_cup__second_round_fixtures"
UNION ALL
SELECT * FROM "2024_25_german_cup__round_of_16_fixtures"
UNION ALL
SELECT * FROM "2024_25_german_cup__quarterfinals_fixtures"
UNION ALL
SELECT * FROM "2024_25_german_cup__semifinals_fixtures"
UNION ALL
SELECT * FROM "2024_25_german_cup__final_fixtures";

CREATE OR REPLACE VIEW german_top_flight AS
SELECT "eventId", "matchDate", "HomeTeam", "AwayTeam", "HomeTeamScore", "AwayTeamScore", 'Bundesliga' AS Competition
FROM "2024_25_german_bundesliga_fixtures"
UNION ALL
SELECT "eventId", "matchDate", "HomeTeam", "AwayTeam", "HomeTeamScore", "AwayTeamScore", 'DFB Pokal' AS Competition
FROM dfbpokal_fixtures
UNION ALL
SELECT "eventId", "matchDate", "HomeTeam", "AwayTeam", "HomeTeamScore", "AwayTeamScore", 'German Super Cup' AS Competition
FROM "2024_german_super_cup_fixtures";


SELECT
    * FROM german_top_flight
WHERE
    "HomeTeam" IN (SELECT "displayName" FROM "2024_25_german_bundesliga_standings")
   OR "AwayTeam" IN (SELECT "displayName" FROM "2024_25_german_bundesliga_standings");




DROP VIEW IF EXISTS v_german_plays_data CASCADE;
CREATE OR REPLACE VIEW v_german_plays_data AS
    --
-- Bundesliga
SELECT *, 'Bundesliga' AS Competition
FROM "2024_25_german_bundesliga_plays_data" UNION ALL
-- DFB Pokal
SELECT *, 'DFB Pokal' AS Competition
FROM "2024_25_german_cup___plays_data" UNION ALL
-- German Super Cup
SELECT *, 'German Super Cup' AS Competition
FROM "2024_german_super_cup_plays_data";
...
FROM
    german_top_flight
        INNER JOIN
    v_german_plays_data
    ON german_top_flight."eventId" = v_german_plays_data."eventId"
WHERE
    ("HomeTeam" IN (SELECT "displayName" FROM "2024_25_german_bundesliga_standings")
        OR "AwayTeam" IN (SELECT "displayName" FROM "2024_25_german_bundesliga_standings"))
  AND ("playDescription" LIKE 'Goal%' OR "playDescription" LIKE 'Penalty - Scored') AND
    team in (SELECT "displayName" FROM "2024_25_german_bundesliga_standings");
...
SELECT
    ...
    "team",
    COUNT(*) as goal_count
FROM v_german_plays_data
WHERE "playDescription" LIKE 'Goal%'
  AND "Assister" IS NOT NULL
  AND "team" IN (SELECT "displayName" FROM "2024_25_german_bundesliga_standings")
...
FROM german_top_flight
WHERE german_top_flight.Competition = 'Bundesliga'
GROUP BY "HomeTeam"
...
FROM
    v_german_plays_data s  JOIN v_german_plays_data g -- The Goal Event
...
WHERE
  ...
  AND s.team IN (SELECT "displayName" FROM "2024_25_german_bundesliga_standings")
GROUP BY
...
WITH Goal_Context AS (
    ...
    FROM v_german_plays_data p
             JOIN german_top_flight m ON p."eventId" = m."eventId"
    WHERE (p."playDescription" LIKE 'Goal%' OR p."playDescription" = 'Penalty - Scored')
      AND p.team IN (SELECT "displayName" FROM "2024_25_german_bundesliga_standings")
)
