CREATE OR REPLACE VIEW uecl_fixtures AS
SELECT * FROM "2024_25_uefa_conference_league__league_phase_fixtures"
UNION ALL
SELECT * FROM "2024_25_uefa_conference_league__round_of_16_fixtures"
UNION ALL
SELECT * FROM "2024_25_uefa_conference_league__quarterfinals_fixtures"
UNION ALL
SELECT * FROM "2024_25_uefa_conference_league__semifinals_fixtures"
UNION ALL
SELECT * FROM "2024_25_uefa_conference_league__final_fixtures";

CREATE OR REPLACE VIEW uecl_top_flight AS
SELECT "eventId", "matchDate", "HomeTeam", "AwayTeam", "HomeTeamScore", "AwayTeamScore", 'UEFA Europa League' AS Competition
FROM uecl_fixtures;


DROP VIEW IF EXISTS v_uecl_plays_data CASCADE;
CREATE OR REPLACE VIEW v_uecl_plays_data AS

SELECT *, 'UEFA Conference League' AS Competition
FROM "2024_25_uefa_conference_league__plays_data";
...
FROM
    uecl_top_flight
        INNER JOIN
    v_uecl_plays_data
    ON uecl_top_flight."eventId" = v_uecl_plays_data."eventId"
WHERE
    ("HomeTeam" IN (SELECT "displayName" FROM "2024_25_uefa_conference_league__league_phase_standings")
        OR "AwayTeam" IN (SELECT "displayName" FROM "2024_25_uefa_conference_league__league_phase_standings"))
  AND ("playDescription" LIKE 'Goal%' OR "playDescription" LIKE 'Penalty - Scored') AND
    team in (SELECT "displayName" FROM "2024_25_uefa_conference_league__league_phase_standings");
...
SELECT
    ...
    "team",
    COUNT(*) as goal_count
FROM v_uecl_plays_data
WHERE "playDescription" LIKE 'Goal%'
  AND "Assister" IS NOT NULL
  AND "team" IN (SELECT "displayName" FROM "2024_25_uefa_conference_league__league_phase_standings")
...
FROM "2024_25_uefa_conference_league__league_phase_fixtures"
GROUP BY "HomeTeam"
...
FROM
    v_uecl_plays_data s  JOIN v_uecl_plays_data g -- The Goal Event
...
WHERE
  ...
  AND s.team IN (SELECT "displayName" FROM "2024_25_uefa_conference_league__league_phase_standings")
GROUP BY
...
WITH Goal_Context AS (
    ...
    FROM v_uecl_plays_data p
             JOIN uecl_top_flight m ON p."eventId" = m."eventId"
    WHERE (p."playDescription" LIKE 'Goal%' OR p."playDescription" = 'Penalty - Scored')
      AND p.team IN (SELECT "displayName" FROM "2024_25_uefa_conference_league__league_phase_standings")
)
