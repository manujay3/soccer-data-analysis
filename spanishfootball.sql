CREATE OR REPLACE VIEW copadelrey_fixtures AS
SELECT * FROM "2024_25_spanish_copa_del_rey__qualifying_round_fixtures"
UNION ALL
SELECT * FROM "2024_25_spanish_copa_del_rey__first_round_fixtures"
UNION ALL
SELECT * FROM "2024_25_spanish_copa_del_rey__second_round_fixtures"
UNION ALL
SELECT * FROM "2024_25_spanish_copa_del_rey__round_of_32_fixtures"
UNION ALL
SELECT * FROM "2024_25_spanish_copa_del_rey__round_of_16_fixtures"
UNION ALL
SELECT * FROM "2024_25_spanish_copa_del_rey__quarterfinals_fixtures"
UNION ALL
SELECT * FROM "2024_25_spanish_copa_del_rey__semifinals_fixtures"
UNION ALL
SELECT * FROM "2024_25_spanish_copa_del_rey__final_fixtures";


CREATE OR REPLACE VIEW spanishsupercup_fixtures AS
SELECT * FROM "2024_25_spanish_supercopa__semifinals_fixtures"
UNION ALL
SELECT * FROM "2024_25_spanish_supercopa__final_fixtures";


CREATE OR REPLACE VIEW spanish_top_flight AS
SELECT "eventId", "matchDate", "HomeTeam", "AwayTeam", "HomeTeamScore", "AwayTeamScore", 'La Liga' AS Competition
FROM "2024_25_laliga_fixtures"
UNION ALL
SELECT "eventId", "matchDate", "HomeTeam", "AwayTeam", "HomeTeamScore", "AwayTeamScore", 'Copa Del Rey' AS Competition
FROM copadelrey_fixtures
UNION ALL
SELECT "eventId", "matchDate", "HomeTeam", "AwayTeam", "HomeTeamScore", "AwayTeamScore", 'Spanish SuperCopa' AS Competition
FROM "spanishsupercup_fixtures";


SELECT
    * FROM spanish_top_flight
WHERE
    "HomeTeam" IN (SELECT "displayName" FROM "2024_25_laliga_standings")
   OR "AwayTeam" IN (SELECT "displayName" FROM "2024_25_laliga_standings");




DROP VIEW IF EXISTS v_spanish_plays_data CASCADE;
CREATE OR REPLACE VIEW v_spanish_plays_data AS
    --
--
SELECT *, 'La Liga' AS Competition
FROM "2024_25_laliga_plays_data" UNION ALL
--
SELECT *, 'Copa Del Rey' AS Competition
FROM "2024_25_spanish_copa_del_rey__plays_data" UNION ALL
--
SELECT *, 'Spanish Supercup' AS Competition
FROM "2024_25_spanish_supercopa__plays_data";
...
FROM
    spanish_top_flight
        INNER JOIN
    v_spanish_plays_data
    ON spanish_top_flight."eventId" = v_spanish_plays_data."eventId"
WHERE
    ("HomeTeam" IN (SELECT "displayName" FROM "2024_25_laliga_standings")
        OR "AwayTeam" IN (SELECT "displayName" FROM "2024_25_laliga_standings"))
  AND ("playDescription" LIKE 'Goal%' OR "playDescription" LIKE 'Penalty - Scored') AND
    team in (SELECT "displayName" FROM "2024_25_laliga_standings");
...
SELECT
    ...
    "team",
    COUNT(*) as goal_count
FROM v_spanish_plays_data
WHERE "playDescription" LIKE 'Goal%'
  AND "Assister" IS NOT NULL
  AND "team" IN (SELECT "displayName" FROM "2024_25_laliga_standings")
...
FROM spanish_top_flight
WHERE spanish_top_flight.Competition = 'La Liga'
GROUP BY "HomeTeam"
...
FROM spanish_top_flight
WHERE spanish_top_flight.Competition = 'Serie A'
GROUP BY "AwayTeam"
...
FROM
    v_spanish_plays_data s  JOIN v_spanish_plays_data g -- The Goal Event
...
WHERE
  ...
  AND s.team IN (SELECT "displayName" FROM "2024_25_laliga_standings")
GROUP BY
...
WITH Goal_Context AS (
    ...
    FROM v_spanish_plays_data p
             JOIN spanish_top_flight m ON p."eventId" = m."eventId"
    WHERE (p."playDescription" LIKE 'Goal%' OR p."playDescription" = 'Penalty - Scored')
      AND p.team IN (SELECT "displayName" FROM "2024_25_laliga_standings")
)
